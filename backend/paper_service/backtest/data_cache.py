...
class DataCache:
    ...
    def _evict_lru(self):
        if self._cache: self._cache.pop(next(iter(self._cache)))

    @staticmethod
    def _ts(s: str) -> int: return int(pd.Timestamp(s).timestamp())
    ...
    def _stale(self, df: pd.DataFrame, end_date: str, timeframe: str) -> bool:
        ...
        return df.index[-1] < self._ts(end_date) - self._bar_seconds(timeframe)

    def ensure(self, symbol: str, start_date: Optional[str] = None, end_date: Optional[str] = None, timeframe: Optional[str] = None) -> Optional[pd.DataFrame]:
        ...
        if df is not None:
            if start_date and end_date and timeframe:
                if self._stale(df, end_date, timeframe):
                    ...
                    if mask.any(): return df.loc[mask]
