...
export function EquityCurve({ taskId }: Props) {
  ...
  // Fetch data + render equity curve
  useEffect(() => {
    ...
    fetch(`/tasks/${taskId}/individuals/${sid}/equity-curve`)
      ...
      .then((data) => {
        ...
        if (symbols.length <= 1) {
          traces.push({
            x: xDates.slice(0, cut),
            y: combined.slice(0, cut),
            type: 'scatter', mode: 'lines', name: '訓練集',
            line: { color: '#2563eb' },
          }, {
            x: xDates.slice(cut),
            y: combined.slice(cut),
            type: 'scatter', mode: 'lines', name: '驗證集',
            line: { color: '#dc2626' },
          })
        } else {
          ...
          if (combined.length > 0) {
            traces.push({
              x: xDates.slice(0, combined.length),
              y: combined,
              type: 'scatter', mode: 'lines', name: '加權平均',
        ...
        if (dcaCombined.length > 0) {
          traces.push({
            x: pxDates.slice(0, dcaCombined.length),
            y: dcaCombined,
  ...
  // Render per-symbol price charts when divs are in DOM
  useEffect(() => {
