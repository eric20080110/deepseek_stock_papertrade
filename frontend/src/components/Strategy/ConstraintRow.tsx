...
import type { ConstraintDef, ParamDef, CategoricalParam } from '../../types/strategy'
    ...
    return (
      <div className="flex items-center gap-2 py-2 px-3 border rounded-lg bg-white text-sm">
        ...
        <select
          ...
          onChange={(e) => onChange({ ...constraint, relation: e.target.value as 'less_than' | 'less_equal' })}
        ...
        <select
          ...
          onChange={(e) => onChange({ ...constraint, repair: e.target.value as 'clamp_upper' | 'clamp_lower' | 'swap' })}
          className="px-1 py-1 border rounded text-xs"
        >
    ...
    return (
      <div className="flex items-center gap-2 py-2 px-3 border rounded-lg bg-white text-sm">
        ...
        <label className="text-xs">
            控制器
           <select value={constraint.controller} onChange={(e) => onChange({ ...constraint, controller: e.target.value })} className="ml-1 px-1 py-1 border rounded text-xs">
            <option value="">--</option>
        ...
        <label className="text-xs">
          啟用條件
          <select value={String(constraint.active_when)} onChange={(e) => onChange({ ...constraint, active_when: e.target.value === 'true' } as ConstraintDef)} className="ml-1 px-1 py-1 border rounded text-xs">
        ...
        <label className="text-xs">
          受控參數
          <select multiple value={constraint.controlled_params} onChange={(e) => onChange({ ...constraint, controlled_params: Array.from(e.target.selectedOptions, (o) => o.value) } as ConstraintDef)} className="ml-1 px-1 py-1 border rounded text-xs" size={2}>
    ...
    return (
      <div className="flex flex-col gap-2 py-2 px-3 border rounded-lg bg-white text-sm">
        <div className="flex items-center gap-2">
          ...
          <label className="text-xs">
            控制器
            <select value={constraint.controller} onChange={(e) => onChange({ ...constraint, controller: e.target.value, groups: {} } as ConstraintDef)} className="ml-1 px-1 py-1 border rounded text-xs">
              <option value="">--</option>
        ...
        {constraint.controller && params.find((p) => p.name === constraint.controller)?.type === 'categorical' && (
          <div className="ml-16 space-y-1">
            {(() => {
              ...
              const opts = catParam && 'options' in catParam ? (catParam as CategoricalParam).options : []
              return opts.map((opt: string) => (
                <div key={opt} className="flex items-center gap-2 text-xs">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const selected = Array.from(e.target.selectedOptions, (o) => o.value)
                      onChange({ ...constraint, groups: { ...constraint.groups, [opt]: selected } } as ConstraintDef)
                    }}
