...
def continue_instance(instance_id: str):
    ...
    req = CreateInstanceRequest(
        ...
        source_task_id=inst.source_task_id, source_individual_id=inst.source_individual_id,
        ...
        initial_capital=inst.initial_capital, timeframe=inst.timeframe,
    )
    try:
...
def get_chart(instance_id: str):
    ...
    result["trades"] = [{"symbol": t["symbol"], "side": t["side"], "price": t["price"], "time": t["executed_time"], "pnl": t.get("realized_pnl")} for t in raw_trades]
    return result
...
def binance_webhook(payload: dict):
    ...
    if not bar: return {"ok": False, "message": "Missing kline data"}
    ...
    for inst in instances:
        ...
        if symbol and symbol not in syms: continue
